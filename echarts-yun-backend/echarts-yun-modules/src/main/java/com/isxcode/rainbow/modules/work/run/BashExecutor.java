...
import static com.isxcode.rainbow.common.utils.ssh.SshUtils.*;
    ...
    public void execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance) {
        ...
        try {
            ...
            scpText(scpFileEngineNodeDto, workRunContext.getScript() + "\necho 'zhiqingyun_success'",
                clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/" + workInstance.getId() + ".sh");

            // 执行命令获取pid
            String executeBashWorkCommand = "nohup sh " + clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/"
                + workInstance.getId() + ".sh >> " + clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/"
            ...
            if (InstanceStatus.RUNNING.equals(pidStatus)) {
                ...
                try {
                    String clearWorkRunFile = "rm -f " + clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/"
                        ...
                        + "/zhishiyun-agent/works/" + workInstance.getId() + ".sh";
