...
import com.isxcode.rainbow.common.utils.ssh.SshUtils;
...
import static com.isxcode.rainbow.common.utils.ssh.SshUtils.scpText;
    ...
    public void execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance) {
        ...
        try {
            ...
            scpText(scpFileEngineNodeDto, workRunContext.getScript() + "\nprint('zhiqingyun_success')",
                clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/" + workInstance.getId() + ".py");

            // 执行命令获取pid
            String executeBashWorkCommand = "nohup python3 " + clusterNode.getAgentHomePath()
                + "/zhishiyun-agent/works/" + workInstance.getId() + ".py >> " + clusterNode.getAgentHomePath()
                + "/zhishiyun-agent/works/" + workInstance.getId() + ".log 2>&1 & echo $!";
        ...
        while (true) {
            ...
            if (InstanceStatus.RUNNING.equals(pidStatus)) {
                ...
                String getLogCommand =
                    "cat " + clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/" + workInstance.getId() + ".log";
                String logCommand = "";
                try {
                ...
                // 删除脚本和日志
                try {
                    String clearWorkRunFile = "rm -f " + clusterNode.getAgentHomePath() + "/zhishiyun-agent/works/"
                        + workInstance.getId() + ".log && " + "rm -f " + clusterNode.getAgentHomePath()
                        + "/zhishiyun-agent/works/" + workInstance.getId() + ".py";
